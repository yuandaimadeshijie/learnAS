...
public class AnimatedGifDrawable extends AnimationDrawable {
	...
	public AnimatedGifDrawable(Resources res, int size, InputStream source, UpdateListener listener) {
		mListener = listener;
		...
		for (int i = 0; i < decoder.getFrameCount(); i++) {
			...
			if (size > 0)
				drawable.setBounds(4, 0, size + 8, size);
			else
				drawable.setBounds(4, 0, bitmap.getWidth() + 8, bitmap.getHeight());
